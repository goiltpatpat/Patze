...
export function apply(ctx: CordisContext, config: JevConfig = {}) {
  ...
  if (typeof ctx?.on === 'function') {
    ...
    ctx.on('agent/pre-step', async ({ agent, messages }: any, next: () => Promise<any>) => {
      const decision = await next()
      ...
      try {
        if (Array.isArray(messages)) {
          const userMsg = typeof messages.findLast === 'function'
            ? messages.findLast((m: any) => m?.source?.kind === 'user' || m?.role === 'user')
            : messages[messages.length - 1]

          let userText = ''
          if (Array.isArray(userMsg?.content)) {
            userText = userMsg.content.map((c: any) => c?.text || '').join(' ')
